...
## Other parameters
## independent of codalab submission, used for init_checkpoint on prediction
flags.DEFINE_string("example_file", "KorQuAD_v1.0_dev.json",
                    "The vocabulary file that the BERT model was trained on.")
...
flags.DEFINE_bool(
    "analyze_kor_morph", True,
    "Use Mecab in konlpy to analyze morpheme of Korean text. Need to install"
    "konlpy and Mecab.")
...
def input_fn_builder(input_data, tokenizer, seq_length, drop_remainder, question):
    ...
    def input_fn():
        return (tf.data.Dataset.from_generator(gen,
                                               output_types={'unique_ids': tf.int32, 'input_ids': tf.int32,
                                                             'input_mask': tf.int32, 'segment_ids': tf.int32},
                                               output_shapes={'unique_ids': [None], 'input_ids': [None, seq_length],
...
def write_predictions(all_examples, all_features, all_results, n_best_size,
    ...
    return nbest_json[0]["text"], nbest_json[0]['start_logit'] + nbest_json[0]['end_logit'], nbest_json[0][
        'probability']
